...
Your phone plays one role at a time in the real bot (donor, org or individual). This script plays
the others, on the same database and through the same rules the app uses:
...
def my_roles(ctx: Ctx) -> dict:
    return {
        "donor": repo.get_donor_by_chat(ctx.chat_id),
        "org": repo.get_org(ctx.chat_id, ctx.bot),
        "individual": repo.get_individual(ctx.chat_id, ctx.bot),
    }
...
def cmd_post(ctx: Ctx, a) -> None:
    org = my_roles(ctx)["org"]
    if not org:
        sys.exit(f"You have no org on @{ctx.bot} yet, so there's nowhere to post near. Register one first.")
    ...
    if a.far:
        ...
        print("Your org's alert should arrive in Telegram within a few seconds.")
