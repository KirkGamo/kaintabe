...
| 7 | 4:10 | Telegram + map | Send `/demo individual`. Terminal: `demo post --far`, then `demo escalate`. Click **🙋 I'll pick it up**. | The circle jumps to 8 km, and a **flash offer** arrives | "If no kitchen claims it, the search widens every few minutes. At the limit, nearby individuals who signed up get a flash offer. Food doesn't sit." |
